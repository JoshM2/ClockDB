...
export default {
  async scheduled(controller, env, ctx) {
      ...
      if (result.type=="average") {
        ...
        data.forEach(recon => {
          ...
          if (recon['wcaid'] == result['person_id'] && found==false) {
            // right time
            if (recon["title"].includes("Average") && recon['title'].split(" ")[0] == ""+(result['value']/100).toFixed(2)) {
              let firstSolveNum = JSON.parse(recon.average)[0][0]
              addToTable(convertedDate,result.person_name,(result.value/100).toFixed(2),`<a href="https://clockdb.net/r/${firstSolveNum}">https://clockdb.net/r/${firstSolveNum}</a>`)
              found=true;
              found_counter += 1;
            }
          }
    ...
    await env.KV.put("wr-history-average",rankingsTable)
