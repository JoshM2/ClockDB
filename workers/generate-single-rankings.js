export default {
  async scheduled(controller, env, ctx) {
    const response = await fetch("https://www.worldcubeassociation.org/results/rankings/clock/single?", {
        "headers": {
    ...
    single_rankings['rows'].forEach(result => {
      ...
      solveNum += 1;
      if (result.best != lastTime) {
        rank = solveNum;
      }
      lastTime = result.best;
      // top 100 result
      if (rank <= 100) {
        let found = false;
        counter += 1
        data.forEach(recon => {
          ...
          if (recon['wcaid'] == result['person_id'] && found==false) {
            // right time
            if (JSON.parse(recon['solveStart'])[1] == (result['best']/100).toFixed(2)) {
              // console.log(result['best'],recon['num'])
              addToTable(rank,result.person_name,(result.best/100).toFixed(2),`<a href="https://clockdb.net/r/${recon.num}">https://clockdb.net/r/${recon.num}</a>`)
              found=true;
              found_counter += 1;
            }
          }
        ...
        if (found == false) {
          addToTable(rank,result.person_name,(result.best/100).toFixed(2),"none")
        }
      }     
    })
    ...
    await env.KV.put("rankings-single",rankingsTable)
